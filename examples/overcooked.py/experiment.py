...
class MultiAgentTimestepWrapper(object):
    ...
    def reset(
        ...
        timestep = nicewebrl.TimeStep(
            ...
            step_type=jnp.full(shape, nicewebrl.StepType.FIRST,
                               dtype=nicewebrl.StepType.FIRST.dtype),
        )
        return timestep
...
class MultiAgentJaxWebEnv:
    def __init__(self, env, actions):
        ...
        def next_steps(rng, timestep, env_params, other_action=4, h_id=0):
            ...
            timesteps = jax.vmap(step_env, in_axes=(None, None, 0, 0, None), out_axes=0)(
                rng, timestep, actions, actions, env_params)
            return timesteps
    ...
    def precompile_vmap_render_fn(
            self,
            render_fn,
            dummy_env_params: Optional[struct.PyTreeNode] = None):
...
class CheckBoxes:
  ...
  async def make(self):
    ...
    with ui.column():
      ui.label("Press any key when ready to continue to next section")
      for user_id in room_users:
        with ui.row():
          status = "ready" if checked.get(str(user_id), False) else "not ready"
          color = "green" if checked.get(str(user_id), False) else "red"
          ui.markdown(f"{user_id}: **<span style='color: {color}'>{status}</span>**")
...
async def instruction_display_fn(stage, container):
  ...
  with container.style('align-items: center;'):
    ...
    broadcast_message('update_label', 'false')

    room_users = get_room_users()
    ui.markdown("press a key when ready. when both participants press a key, advance to next stage.")
    labels = {}
    for r in room_users:
    ...
    async def handle_key_press(event, c):
      ...
      if sum(checked.values()) == 2:
        ...
        broadcast_message.refresh('stage_over', 'true')
