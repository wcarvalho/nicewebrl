...
def fill_coords(img, fn, color):
    y, x = jnp.meshgrid(jnp.arange(img.shape[0]), jnp.arange(
...
def render_tile(obj, highlight, agent_dir_idx, agent_inv, tile_size=TILE_PIXELS):
    ...
    def render_obj(img, obj_type, color):
        ...
        def render_agent(img):
            ...
            tri_fn = rotate_fn(tri_fn, 0.5, 0.5, 0.5 *
                               jnp.pi * agent_idx_dir_map[agent_dir_idx])
            img = fill_coords(img, tri_fn, color)

            # Render inventory item
            def render_inventory(img):
                def _render_nothing(x): return x
                ...
                def _render_mini_dish(img):
                    ...
                    return img
                img = lax.cond(
                    agent_inv == OBJECT_TO_INDEX['onion'], _render_mini_onion, _render_nothing, img)
                img = lax.cond(
                    agent_inv == OBJECT_TO_INDEX['plate'], _render_mini_plate, _render_nothing, img)
                img = lax.cond(
                    agent_inv == OBJECT_TO_INDEX['dish'], _render_mini_dish, _render_nothing, img)
...
def rendering_pot(obj, img):
    ...
    def render_onions(img, num_onions):
        ...
        def render_onion(carry, i):
            ...
            img = jnp.where(i < num_onions, fill_coords(
                img, onion_fn, COLORS["yellow"]), img)  # show progress of onions filled
            return (img, coords, num_onions), None
        carry, _ = lax.scan(render_onion, (img, onion_fns,
                            num_onions), jnp.arange(len(onion_fns)))
        img = carry[0]
        return img
    ...
    img = lax.cond(
        is_done,
        lambda x: fill_coords(x, point_in_rect(
            0.12, 0.88, 0.23, 0.35), COLORS["orange"]),
        lambda x: x,
        img
    )
    ...
    img = lax.cond(
        is_cooking,
        lambda x: fill_coords(x, point_in_rect(
            0.1, 0.9-(0.9-0.1)/20*pot_status, 0.83, 0.88), COLORS["green"]),
        lambda x: x,
        img
    )
...
def render_grid(grid, highlight_mask, agent_dir_idx, agent_inv, tile_size=TILE_PIXELS):
    ...
    img = jnp.zeros((height * tile_size, width *
                    tile_size, 3), dtype=jnp.uint8)
