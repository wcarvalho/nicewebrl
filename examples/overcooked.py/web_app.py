...
def args_from_room(args):
  ...
  if user_room != str(args['called_by_room_id']):
    logger.info(
        f"user room {user_room} != called by room {args['called_by_room_id']}")
    return False

  if args['stage'] != str(app.storage.user['stage_idx']):
    logger.info(
        f"user stage {app.storage.user['stage_idx']} != called by stage {args['stage']}")
    return False
...
async def run_stage(stage, container):
  ...
  await stage.set_user_data(stage_over_event=stage_over_event)
...
def add_user_to_room():
  ...
  app.storage.general['latest_room_id'] = app.storage.general.get('latest_room_id', None)
  if not rooms:
    latest_room_id = str(random.getrandbits(32))
    rooms[latest_room_id] = []
