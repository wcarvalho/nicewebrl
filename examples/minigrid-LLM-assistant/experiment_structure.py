...
class NavixTimestepWrapper(TimestepWrapper):
  ...
  def step(self, key, state, action, params=None):
    ...
    return TimeStep(
      ...
      step_type=jnp.where(
        timestep.is_done(), jnp.array(2, dtype=jnp.uint8), jnp.array(1, dtype=jnp.uint8)
      ),
    )


# Wrap environment
jax_env = NavixTimestepWrapper(jax_env, autoreset=True, use_params=True)
...
def render_fn(timestep: nicewebrl.TimeStep):
  ...
  return jax.image.resize(
    timestep.observation, shape=(256, 256, 3), method="nearest"
  ).astype(jnp.uint8)
