...
async def index(request: Request):
  ...
  # Show model selection if not already chosen
  if "selected_model" not in app.storage.user:
    with ui.dialog() as dialog, ui.card():
      ui.label("Choose your AI Assistant").classes("text-2xl mb-4")
      ui.label("Select which AI you'd like to help you through the game:").classes("mb-4")
      
      with ui.row().classes("w-full justify-center gap-4"):
        for model in model_list:
          with ui.card().classes("w-48 cursor-pointer hover:bg-gray-100").on("click", lambda m=model: select_model(m)):
            ui.label(model.upper()).classes("text-xl font-bold")
            if model == "gemini":
              ui.label("Google's latest AI model").classes("text-sm text-gray-600")
            elif model == "claude":
              ui.label("Anthropic's advanced AI").classes("text-sm text-gray-600")
            else:
              ui.label("OpenAI's conversational AI").classes("text-sm text-gray-600")
      
      ui.label("Click on your preferred AI to begin").classes("text-sm text-gray-500 mt-4")
      
      async def select_model(model):
        app.storage.user["selected_model"] = model
        dialog.close()
        # Start the experiment after model selection
        asyncio.create_task(start_experiment(gameplay_container))
    
    dialog.open()
  else:
    # If model is already selected, start the experiment
    asyncio.create_task(start_experiment(gameplay_container))
