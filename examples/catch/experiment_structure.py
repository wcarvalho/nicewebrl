...
actions = [Actions.left, Actions.noop, Actions.right]
...
rows = 10
cols = 5
jax_env = catch.Catch(rows=rows, columns=cols)
...
default_params = jax_env.default_params
...
def render_fn(timestep: nicewebrl.TimeStep):
  image = catch.render(timestep.state, nrows=rows, ncols=cols)
  return image
...
async def instruction_display_fn(stage, container):
  with container.style("align-items: center;"):
    ...
    ui.markdown("""Press the arrow keys to move the paddle"""
    )
